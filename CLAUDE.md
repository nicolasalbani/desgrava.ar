...
- `invoices/` — Pure helpers shared between the ARCA pull and the manual create/upload paths. `dedupe.ts` exports `invoiceDedupeKey(providerCuit, invoiceNumber, fiscalYear)` (the natural-identity key used to deduplicate on import and to find an overwrite target on re-upload) and `shouldSoftDelete(source)` (true for `ARCA`, false otherwise — drives the soft-delete branch in `DELETE /api/comprobantes/[id]` so re-pulling skips user-deleted rows).
...
- `notifications/` — Pure helpers and post-job email dispatcher for the daily ARCA pull. `eligibility.ts` exports `isEligibleForDailyPull` (eligibility filter: active subscription via `resolveCanWrite` + `ArcaCredential.isValidated` + `UserPreference.notifications`) and `shouldSendNotificationToday` (start-of-UTC-day comparison). `notify-pull-completion.ts` exports `notifyPullCompletion(userId, "comprobantes" | "recibos", newDeducibleCount)` — invoked at the end of `processPullComprobantes` / `processPullDomesticReceipts` when `AutomationJob.notifyOnComplete = true`, sends `sendNewDeductibleInvoicesEmail` / `sendNewDeductibleReceiptsEmail`, and updates `UserPreference.lastComprobantesNotifiedAt` / `lastRecibosNotifiedAt` for the per-type per-UTC-day throttle.
- `mercadopago/` — MercadoPago SDK integration: client init, preapproval (subscription) creation/cancellation, webhook processing
- `github/` — GitHub Issues integration. `issues.ts` exports `createSupportIssue({ subject, description, userEmail, pageUrl, automationJobId, conversationLog })` which POSTs `https://api.github.com/repos/${GITHUB_REPO}/issues` (Bearer token auth, label `from-app`) and returns `{ number, url }`. Plain `fetch`, no Octokit. Throws on missing `GITHUB_TOKEN` / `GITHUB_REPO` env vars and on non-2xx responses. `buildIssueBody` (also exported) is a pure helper used by tests.
...
- **Invoice management** uses Dialog modals for upload and manual entry, with multi-select popover filters (categories, statuses) on the table view. Both facturas and recibos use server-side pagination via `usePaginatedFetch` — filtering, search, and pagination are handled by the API with Prisma `skip`/`take` and `count()` in parallel. **Delete behavior is source-dependent**: ARCA-sourced comprobantes (`source = "ARCA"`) soft-delete (the DELETE handler updates `deductionCategory` to `NO_DEDUCIBLE` and clears `familyDependentId`, keeping the row so the next `PULL_COMPROBANTES` skips it via the existing dedup check); manual/PDF/OCR/email rows hard-delete as usual (cascade `AutomationJob`s + best-effort `deleteFile`). Recovery: re-uploading via the manual form or PDF flow matches by full dedup key (`providerCuit + invoiceNumber + fiscalYear` — `invoiceNumber` required for the match) and overwrites an existing `NO_DEDUCIBLE` row in-place; a non-`NO_DEDUCIBLE` match still returns `409`. The shared `shouldSoftDelete` / `invoiceDedupeKey` helpers live in [src/lib/invoices/dedupe.ts](src/lib/invoices/dedupe.ts) and are re-exported from `src/lib/automation/csv-parser.ts` for the pull processor.
...
- **Support chat (Ganancio)** persists every chat as a `SupportConversation` row (`messages` JSON, `title?` AI-generated after 4 turns, `lastMessageAt` indexed). The server is the canonical store — `POST /api/soporte/chat` accepts `{ message, conversationId? }` and on each turn loads/creates the conversation, runs OpenAI with the last 40 turns of stored history (older turns stay in the DB but drop from the model context), persists user+assistant messages atomically, and returns `{ conversationId, content, events }`. **Tickets escalate to GitHub Issues** (label `from-app` on the project repo). When the `create_ticket` tool fires, the chat route calls [src/lib/github/issues.ts](src/lib/github/issues.ts) `createSupportIssue` (POSTs the user's subject + description + a collapsible conversation log to GitHub), stamps `SupportConversation.githubIssueNumber` + `githubIssueUrl` for the 1:1 mapping, and posts a Telegram notification via `sendNewGithubIssueNotification`. Calling `create_ticket` again on the same conversation returns `success: false` with a "iniciá una nueva conversación" message that the system prompt instructs Ganancio to surface. **No Postgres ticket table** — `SupportTicket` was removed in favor of GitHub as the single source of truth. The chat panel header has a list/back icon that toggles to a `ConversationsList` view (sorted by `lastMessageAt`, ticket badge when `githubIssueNumber !== null`, relative-date labels via `Intl.RelativeTimeFormat`) and a `+` icon to start a new chat. The `useConversation` hook is hosted at `SupportChatButton` so the active `conversationId` survives close/reopen of the panel; full page reload starts a fresh session. Greeting is client-rendered with the session's first name: `"Hola {firstName}, soy Ganancio, tu asistente de desgrava.ar..."`. Bot avatar is `/ganancio.png` rendered through `<GanancioAvatar size>` (used in message bubbles and the typing indicator); the floating launcher keeps a generic lucide icon. **PR-opened Telegram notifications**: `.github/workflows/notify-pr-opened.yml` runs on every `pull_request: opened|reopened`, walks `closingIssuesReferences` (resolved by GitHub from `Closes #N` in the PR body), and for each referenced issue with the `from-app` label posts a "Fix listo para review" Telegram message inline (no app code involved). The workflow needs `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` as repo secrets. The `from-app` label must be created once via `gh label create from-app --color FF6B35` on the repo.
...
- `invoices/` — dedupe key + soft-delete decision helper (11 tests)
...
- `soporte/` — system prompt content (incl. Ganancio identity + duplicate-ticket guardrail), tool definitions, conversation-title helper, types
- `github/` — GitHub Issues integration: `createSupportIssue` (mocked fetch, env-var error paths, request body shape) + `buildIssueBody` (description + collapsible conversation log)
- `email` — `sendNewDeductibleInvoicesEmail` / `sendNewDeductibleReceiptsEmail`
- `telegram` — Telegram Bot API notifications: formatting, env var handling, error handling, GitHub-issue and PR-ready notifications
...
**Cron schedule** (`vercel.json` `crons`): `sweep-stuck-jobs` every 5 min, plus four daily ones — `daily-pull` at 06:00 UTC, `presentaciones` at 08:00 UTC, `subscription-reminders` at 09:00 UTC, `review-non-deductible` at 10:00 UTC. Each cron route exports both `POST` and `GET` and accepts either auth scheme (`x-cron-secret` or `Authorization: Bearer <CRON_SECRET>`), via [src/lib/cron-auth.ts](src/lib/cron-auth.ts) `verifyCronAuth`. Vercel Cron requires the **Pro** plan for 4+ entries (Hobby caps at 2). All handlers are idempotent — `presentaciones` skips users with an active job; `sweep-stuck-jobs` uses a CAS `updateMany` on `status="RUNNING"`; `daily-pull` skips users with an in-flight `PULL_COMPROBANTES`/`PULL_DOMESTIC_RECEIPTS` job per-type. `daily-pull` enqueues `PULL_COMPROBANTES` for every eligible user (`isEligibleForDailyPull` in [src/lib/notifications/eligibility.ts](src/lib/notifications/eligibility.ts)) and `PULL_DOMESTIC_RECEIPTS` only for users with at least one registered `DomesticWorker`. Both jobs are created with `AutomationJob.notifyOnComplete = true`; the job processor reads that flag at COMPLETED time and fires `notifyPullCompletion` to email the user (separate CTA to `/comprobantes` vs `/recibos`) when there are new deducible rows. Per-type per-UTC-day throttle via `UserPreference.lastComprobantesNotifiedAt` / `lastRecibosNotifiedAt`. The manual "Importar desde ARCA" button keeps `notifyOnComplete = false` (default) so manual pulls never email.
...
- `/fix-ticket [--env dev|prod]` — Scheduled bug fix agent: lists open GitHub issues labeled `from-app`, classifies bugs via AI, fixes them with `/fix-bug`, pushes `fix/issue-<n>` branches and opens PRs whose body contains `Closes #<n>` (so GitHub auto-closes the issue on merge and the `notify-pr-opened.yml` workflow fires the Telegram "fix ready for review" message). `--env prod` syncs the prod DB locally first via `pg_dump`/`pg_restore` so the fix can reproduce against real data.
